...
function pageld(e){
 ...
 if(e.id=="p1b") 
    ...
    //ppp.style.display= "none";
     ...
     {
      showAlert( "Selected (recommended) list is not ready yet.");//glis(url2+"List00.json");
     }
}
    ...
    if(TaskToHandle==1)
      ...
      Telegram.WebApp.openLink(st,{try_instant_view:false});
          ...
          { 
            ...
            Telegram.WebApp.openLink('https://mcoin2025.github.io/Apps/Downloads/',{try_browser:'chrome'});    
            
          }   
  else if(TaskToHandle==4)
          { 
               Telegram.WebApp.openLink('https://www.youtube.com/channel/UCmhREBhPY_7KiHTdirKNT2g',{try_browser:'chrome'});
         ...
         Telegram.WebApp.openTelegramLink('https://t.me/+DCN-okIeLEtkZjlk');
          ...
          else
          {
             if(TaskToHandle==1)
          { 
            Telegram.WebApp.openLink(moviePage,{try_browser:'chrome'});
...
var weekdt = new Date("5/March/2025 20:30:00");
var now = new Date();
 ...
 //Params = new URLSearchParams(window.location.search);
     ...
     /*else if(Params.includes('yts'))
  {
    ...
    //if(vidCodes.includes(p2))Award( 200000);
        ...
        else if(Params.includes('ref' ))
  {
    let [p1,p2,p3,p4]=Params.split('_');
     
     if(p2 != undefined)
    { 
      fl=p2; glis(url2+"List"+fl+".json"); movIndex =p3; 
     ShowMovieList(); ldvid(lis[p4].HLink.replace("dlmania","maniadl" ));
     }           
...
{   let n='',id2= window.Telegram.WebApp.initDataUnsafe.user.id.toString();
  ...
  for(let i=0;i<id2.length;i++)
   ...
   n=n+ en[id2[i]];
         ...
         return 'https://t.me/MCoin2025Bot/MCoin?startapp=' + n;
}
 ...
 function ini( tit, des,img,len,hre='tttt',lre, hsi,lsi,Lid ,Hid)
{
 ...
 +'<div id="len"style="font-size:4vh"> Duration : '+len+ '</div>'
     ...
     +'<button class="bt3" style="color: grey;background: #332e2e;border:1px solid grey;" onclick="showmovie(this);" id="share" name= '+Hid+'>Share Video </button>'
